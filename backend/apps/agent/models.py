...
class AgentInterfaceFeedback(models.Model):
    """用户确认过的步骤-接口匹配反馈，用于后续候选加权。"""

    project = models.ForeignKey(
        'api_debug.Project', on_delete=models.CASCADE,
        related_name='agent_interface_feedbacks', verbose_name='项目',
    )
    interface = models.ForeignKey(
        'api_debug.ApiInterface', on_delete=models.CASCADE,
        related_name='agent_feedbacks', verbose_name='接口',
    )
    step_text = models.CharField('步骤文本', max_length=255)
    normalized_step_text = models.CharField('归一化步骤文本', max_length=255, db_index=True)
    reason = models.TextField('确认原因', blank=True, default='')
    confirm_count = models.PositiveIntegerField('确认次数', default=1)
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL,
        related_name='agent_interface_feedbacks', verbose_name='创建人', null=True, blank=True,
    )
    created_at = models.DateTimeField('创建时间', auto_now_add=True)
    ...
    def __str__(self):
        return f'{self.step_text} -> {self.interface_id}'
