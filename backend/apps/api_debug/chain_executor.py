...
from .diagnostics import ChainFailureDiagnoser
...
def _entry_node_ids(nodes, edges):
    """优先从显式开始节点进入；旧链路兼容全部入度为 0 的节点。"""
    ...
    for edge in edges:
        ...
        if src in in_degree and tgt in in_degree:
            in_degree[tgt] += 1
    return [nid for nid in node_ids if in_degree[nid] == 0]
...
def execute_chain(chain, user, env_vars=None):
    ...
    try:
        # 找入口节点：优先使用显式开始节点，兼容旧链路的全部入度 0 节点
        entry_ids = _entry_node_ids(nodes_list, edges_list)
        if not entry_ids:
        ...
        while queue:
            ...
            if node_type in ('start', 'end'):
                ...
                if node_type == 'end':
                    continue
                for nid in _get_next_nodes(node_id, edges_list):
    ...
    ChainFailureDiagnoser().diagnose(chain, step_results)
