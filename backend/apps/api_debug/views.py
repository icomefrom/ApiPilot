...
@api_view(['POST'])
@permission_classes([AllowAny])
def demo_order_create(request):
    """Demo order creation endpoint for Agent chain examples."""
    payload = request.data if isinstance(request.data, dict) else {}
    sku = payload.get('sku') or 'SKU-1001'
    quantity = int(payload.get('quantity') or 1)
    customer_name = payload.get('customer_name') or 'Demo Customer'
    unit_price = float(payload.get('unit_price') or 99.9)
    order_id = f'ORD-{abs(hash(f"{sku}:{customer_name}")) % 100000:05d}'
    return Response({
        'code': 0,
        'message': 'ok',
        'data': {
...
@api_view(['GET'])
@permission_classes([AllowAny])
def demo_order_detail(request):
    """Demo order detail endpoint for Agent chain examples."""
    order_id = request.query_params.get('order_id') or request.query_params.get('id') or 'ORD-00001'
    return Response({
        'code': 0,
        'message': 'ok',
        'data': {
