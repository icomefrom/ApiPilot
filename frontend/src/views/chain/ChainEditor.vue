<template>
  <div class="chain-editor" ref="editorRef">
    ...
    <div
      ...
      @contextmenu.prevent
    >
      <button class="context-menu-item context-menu-danger" type="button" @click="deleteContextNode">
        {{ t('删除') }}
      </button>
    </div>
  </div>
...
function syncToStoreNow() {
  if (syncTimer) {
    clearTimeout(syncTimer)
    syncTimer = null
  }
  syncingFromProps = true
  emit('update:nodes', getSerializedNodes())
  emit('update:edges', getSerializedEdges())
...
function closeContextMenu() {
  contextMenu.value = { visible: false, nodeId: null, x: 0, y: 0 }
}
...
function onNodeContextMenu(event) {
  ...
  if (!nodeId) return
  openContextMenu(nodeId, rawEvent)
}

defineExpose({ flowNodes, flowEdges, getSerializedNodes, getSerializedEdges, syncToStoreNow })
</script>
