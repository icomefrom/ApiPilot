  ...
  >
    <template v-if="node">
      ...
      <template v-if="node.type === 'interface'">
        <a-form layout="vertical" size="small">
          <a-form-item :label="t('选择接口')">
            <a-select
              v-model:value="node.data.interface_id"
              :placeholder="t('请选择已保存的接口')"
              show-search
        ...
        <div class="panel-section" v-if="selectedInterface">
          <div class="panel-section-header">
            <span>{{ t('变量引用') }}</span>
            <a-tooltip :title="t('用 {{vars.变量名}} 引用上游提取值，{{globals.变量名}} 引用全局参数')">
          ...
          <a-form layout="vertical" size="small">
                ...
                <a-dropdown :trigger="['click']">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key }) => insertVar('url', key)">
                      <a-menu-item v-for="v in availableVars" :key="`vars.${v}`">{{ t('上游变量:') }} {{ v }}</a-menu-item>
                      <a-menu-divider v-if="availableVars.length && globalKeys.length" />
                      <a-menu-item v-for="g in globalKeys" :key="`globals.${g}`">{{ t('全局参数:') }} {{ g }}</a-menu-item>
                    </a-menu>
                  </template>
                </a-dropdown>
              </div>
            ...
            <!-- Headers -->
            <a-form-item :label="t('请求头')" class="compact-form-item">
            ...
            <!-- Query Params -->
            <a-form-item :label="t('参数')" class="compact-form-item">
            ...
            >
              ...
              <div class="body-mode-hint">
                <a-button type="link" size="small" @click="bodyEditMode = 'textarea'">{{ t('切换为完整请求体') }}</a-button>
              </div>
            </a-form-item>
            ...
            >
              <a-textarea
                ...
                :placeholder="selectedInterface.body || t('请求体内容，可使用 {{vars.xxx}} 引用变量')"
                :auto-size="{ minRows: 2, maxRows: 8 }"
                class="script-editor"
                @change="emitChange"
              />
              <div v-if="selectedInterface.body_type === 'json'" class="body-mode-hint">
                <a-button type="link" size="small" @click="bodyEditMode = 'kv'">{{ t('切换为字段编辑') }}</a-button>
              </div>
            </a-form-item>
            <!-- WS Message -->
            <a-form-item v-if="selectedInterface.protocol === 'websocket'" :label="t('WS 消息')" class="compact-form-item">
              <a-textarea
                v-model:value="node.data.overrides.ws_message"
                :placeholder="selectedInterface.ws_message || t('WebSocket 发送消息')"
            ...
            <!-- RPC Method -->
            <a-form-item v-if="selectedInterface.protocol === 'rpc'" :label="t('RPC 方法')" class="compact-form-item">
              <a-input v-model:value="node.data.overrides.rpc_method" :placeholder="selectedInterface.rpc_method || t('方法名')" @change="emitChange" />
            </a-form-item>
          </a-form>
        </div>
        ...
        <div class="panel-section">
          <div class="panel-section-header">
            <span>{{ t('提取规则') }}</span>
            <a-button type="link" size="small" @click="addExtraction"><PlusOutlined /> {{ t('添加') }}</a-button>
          </div>
          <div v-for="(ext, idx) in node.data.extractions" :key="idx" class="extraction-item">
            <a-input v-model:value="ext.var_name" :placeholder="t('变量名')" style="width: 30%" @change="emitChange" />
            <JsonPathPicker
          ...
          <a-empty v-if="!node.data.extractions?.length" :image="simpleImage" :description="t('暂无提取规则')" />
        </div>
        ...
        <div class="panel-section">
          <div class="panel-section-header">
            <span>{{ t('断言规则') }}</span>
            <a-button type="link" size="small" @click="addAssertion"><PlusOutlined /> {{ t('添加') }}</a-button>
          </div>
          ...
          <a-empty v-if="!node.data.assertions?.length" :image="simpleImage" :description="t('暂无断言规则')" />
        </div>
      ...
      <template v-if="node.type === 'condition'">
        ...
        <template v-if="node.data.condition_type === 'variable' || !node.data.condition_type">
          <a-form layout="vertical" size="small">
            <a-form-item :label="t('变量来源')">
              <a-select v-model:value="node.data.source_var" :placeholder="t('选择变量')" @change="emitChange">
                <a-select-option v-for="v in availableVars" :key="v" :value="v">{{ v }}</a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item :label="t('比较符')">
              <a-select v-model:value="node.data.operator" @change="emitChange">
                <a-select-option value="equals">{{ t('等于') }}</a-select-option>
                <a-select-option value="not_equals">{{ t('不等于') }}</a-select-option>
                <a-select-option value="contains">{{ t('包含') }}</a-select-option>
                <a-select-option value="greater_than">{{ t('大于') }}</a-select-option>
                <a-select-option value="less_than">{{ t('小于') }}</a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item :label="t('期望值')">
        ...
        <template v-if="node.data.condition_type === 'jsonpath'">
          ...
          <div class="condition-jsonpath-hint" v-if="!lastExecuteResult">
            <a-alert type="info" :message="t('请先运行链路后，可通过 JSONPath 选择上游接口响应字段')" show-icon />
          </div>
        </template>
        ...
        <template v-if="node.data.condition_type === 'script'">
          <div class="assertion-rule-block">
            <a-textarea
              ...
              class="script-editor"
              :auto-size="{ minRows: 3, maxRows: 12 }"
              :placeholder="conditionScriptPlaceholder"
              @change="emitChange"
            />
            <div class="script-timeout-row">
              <span>{{ t('超时') }}</span>
      ...
      <template v-if="stepResult">
        <a-divider style="margin: 8px 0" />
        <div class="panel-section">
          <div class="panel-section-header"><span>{{ t('执行结果') }}</span></div>
          <a-alert
          ...
          <a-alert
            ...
            style="margin-bottom: 8px"
          />
          <div v-if="stepResult.elapsed_ms" class="result-meta">{{ t('耗时') }}: {{ stepResult.elapsed_ms }}ms</div>
          <div v-if="stepResult.extractions?.length" class="result-meta">
            <div v-for="ext in stepResult.extractions" :key="ext.var_name">
              {{ t('提取') }} {{ ext.var_name }} = {{ JSON.stringify(ext.value) }}
            </div>
          </div>
          ...
          <div v-if="stepResult.condition_detail" class="result-meta">
            ...
            <template v-else-if="stepResult.condition_detail.condition_type === 'jsonpath'">
              ...
              <div>{{ t('实际值') }}: {{ JSON.stringify(stepResult.condition_detail.actual) }} => {{ stepResult.condition_detail.result ? 'True' : 'False' }}</div>
            </template>
            <template v-else-if="stepResult.condition_detail.condition_type === 'script'">
              <div :class="stepResult.condition_detail.pass ? 'result-pass' : 'result-fail'">
                {{ t('脚本') }}: {{ stepResult.condition_detail.pass ? 'True' : 'False' }}
              </div>
          ...
          <template v-if="stepResult.response">
            <a-divider style="margin: 8px 0" />
            <div class="panel-section-header"><span>{{ t('接口返回') }}</span></div>
            <!-- Status -->
            ...
            <template v-if="stepResult.response.body">
              ...
              <div v-if="showRespBody" class="result-body-wrap">
                <a-segmented v-model:value="bodyViewFormat" :options="bodyViewOptions" size="small" style="margin-bottom: 6px" />
...
const ASSERT_TYPE_OPTIONS = computed(() => [
  { value: 'jsonpath', label: 'JSONPath' },
  { value: 'script', label: t('脚本') },
])

const ASSERT_OPERATORS = computed(() => [
  { value: 'equals', label: t('等于') },
  { value: 'not_equals', label: t('不等于') },
  { value: 'contains', label: t('包含') },
  { value: 'greater_than', label: t('大于') },
  { value: 'less_than', label: t('小于') },
  { value: 'exists', label: t('存在') },
  { value: 'not_exists', label: t('不存在') },
])

const CONDITION_TYPE_OPTIONS = computed(() => [
  { value: 'variable', label: t('变量') },
  { value: 'jsonpath', label: 'JSONPath' },
  { value: 'script', label: t('脚本') },
])

const CONDITION_OPERATORS = computed(() => [
  { value: 'equals', label: t('等于') },
  { value: 'not_equals', label: t('不等于') },
  { value: 'contains', label: t('包含') },
  { value: 'not_contains', label: t('不包含') },
  { value: 'greater_than', label: t('大于') },
  { value: 'less_than', label: t('小于') },
  { value: 'exists', label: t('存在') },
  { value: 'not_exists', label: t('不存在') },
...
const bodyViewOptions = computed(() => [
  { value: 'pretty', label: t('格式化') },
  { value: 'raw', label: t('原始') },
])

const scriptPlaceholder = computed(() => t(
  '脚本断言占位符',
  `# 可用变量: vars, globals, response
# vars - 上游提取的变量
# response.status_code / .body / .headers
assert vars.get('token') is not None`,
))

const conditionScriptPlaceholder = computed(() => t(
  '条件脚本占位符',
  `# 可用变量: vars, globals, response
