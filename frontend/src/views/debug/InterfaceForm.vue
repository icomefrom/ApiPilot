<template>
  <div class="interface-form">
    ...
    <a-tabs v-model:activeKey="activeTab" size="small" class="request-tabs">
      ...
      <!-- WebSocket 消息 Tab -->
      <a-tab-pane v-if="store.currentInterface.protocol === 'websocket'" key="ws" :tab="t('消息')">
        <a-textarea
      ...
      <a-tab-pane v-if="store.currentInterface.protocol === 'rpc'" key="rpc" tab="RPC">
        <a-form layout="vertical">
          <a-form-item :label="t('服务名')">
            <a-input v-model:value="store.currentInterface.rpc_service" :placeholder="t('RPC 服务名 (可选)')" />
          </a-form-item>
          <a-form-item :label="t('方法名')">
            <a-input v-model:value="store.currentInterface.rpc_method" :placeholder="t('RPC 方法名')" />
          </a-form-item>
          <a-form-item :label="t('参数 (JSON)')">
            <a-textarea
              v-model:value="store.currentInterface.body"
    ...
    <div class="save-bar">
      ...
      <a-button size="small" @click="handleSave" :loading="saving">
        {{ store.currentInterface.id ? t('更新') : t('保存') }}
      </a-button>
    </div>
  </div>
</template>
...
const urlPlaceholder = computed(() => {
  if (store.currentInterface.protocol === 'websocket') return t('ws:// 或 wss:// 地址')
  if (store.currentInterface.protocol === 'rpc') return t('RPC 服务地址 (HTTP)')
...
async function handleSave() {
  ...
  try {
    ...
    message.success(store.currentInterface.id ? t('更新成功') : t('保存成功'))
    emit('saved')
  } catch {
    // 错误已在拦截器处理
  } finally {
