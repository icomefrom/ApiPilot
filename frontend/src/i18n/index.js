...
assert response.status_code == 200`,
  ...
  // Mock 功能
  'Mock 设置': 'Mock',
  '请先保存接口，再配置 Mock 规则': 'Please save the API first, then configure Mock rules',
  'Mock 已启用': 'Mock enabled',
  'Mock 已禁用': 'Mock disabled',
  '删除规则': 'Delete rule',
  ...
  '响应体': 'Response body',
  '模拟延迟': 'Simulated delay',
  '场景标签': 'Scenario tag',
  '如: default, error, timeout': 'e.g.: default, error, timeout',
  '保存中...': 'Saving...',
  '已自动保存': 'Auto-saved',
  '回显模式：响应体将替换为请求体内容\n若请求无 body，则使用此处内容作为兜底': 'Echo mode: response body will be replaced with request body content.\nIf no request body, use this content as fallback.',
  '模板模式：支持 {{vars.key}} 变量替换\n例如: { "order_id": "{{vars.order_id}}" }': 'Template mode: supports {{vars.key}} variable substitution.\nExample: { "order_id": "{{vars.order_id}}" }',
  'Mock 规则已创建并启用': 'Mock rule created and enabled',
  'Mock 规则已删除': 'Mock rule deleted',
  'Mock 规则': 'Mock rule',
  '已配置且启用': 'Configured & enabled',
  '已配置但未启用': 'Configured but disabled',
  '未配置': 'Not configured',
  '刷新': 'Refresh',
  '该节点将跳过 Mock，发送真实请求': 'This node will skip Mock and send real request',
  '该节点将使用 Mock 响应': 'This node will use Mock response',
  '启用 Mock 规则后，可在此控制是否跳过': 'Enable the Mock rule first, then control skip behavior here',
  '请先在调试页为该接口配置 Mock 规则': 'Please configure a Mock rule for this API on the debug page first',
}
