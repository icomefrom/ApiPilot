<template>
  <a-layout style="min-height: 100vh">
    <a-layout-sider v-model:collapsed="collapsed" collapsible>
      <div class="logo">
        <h2 v-if="!collapsed">TestPilot</h2>
        <h2 v-else>TP</h2>
      </div>
      <a-menu
      ...
      <a-layout-header class="header">
        ...
        <div class="header-right">
          <div class="project-switcher">
            ...
            >
              <a-select-option v-for="project in projectStore.projects" :key="project.id" :value="project.id">
                <span class="project-option">
                  <span class="project-option-name">{{ translateText(project.name) }}</span>
                  <DeleteOutlined
            ...
            <a-button type="link" size="small" @click="openCreateProject" :title="t('新建项目')">
              <PlusOutlined />
            </a-button>
            <a-button type="link" size="small" :disabled="!projectStore.activeProjectId" @click="openMemberManager" :title="t('项目成员')">
              <TeamOutlined />
            </a-button>
          </div>
    ...
    >
      <div class="member-panel">
        <div class="invite-row" v-if="projectStore.canManage">
          ...
          <a-button type="primary" :loading="memberSubmitting" @click="handleInviteMember">{{ t('邀请') }}</a-button>
        </div>
        ...
        >
          <a-table-column :title="t('用户')" data-index="username">
            <template #default="{ record }">
              <div class="member-user">
                <span>{{ record.username }}</span>
                <small>{{ record.email || t('无邮箱') }}</small>
              </div>
            </template>
          </a-table-column>
          <a-table-column :title="t('角色')" data-index="role" width="150">
            <template #default="{ record }">
              <a-select
          ...
          <a-table-column :title="t('加入时间')" data-index="joined_at" width="180">
            <template #default="{ record }">{{ formatDate(record.joined_at) }}</template>
          </a-table-column>
          <a-table-column v-if="projectStore.canManage" :title="t('操作')" width="90">
            <template #default="{ record }">
              <a-button type="link" danger size="small" @click="handleRemoveMember(record)">{{ t('移除') }}</a-button>
            </template>
          </a-table-column>
        </a-table>
      </div>
...
import { locale, setLocale, t, translateText } from '../i18n'
...
function handleRemoveMember(member) {
  Modal.confirm({
    title: t('移除成员'),
    content: `${t('确定要移除')} ${member.username} ${t('吗？')}`,
    async onOk() {
...
function handleDeleteProject(project) {
  ...
  if (isDefaultProject(project)) {
    message.warning(t('默认项目不能删除'))
    return
  }
  Modal.confirm({
    title: t('删除项目'),
    content: `${t('确定要删除项目')}「${translateText(project.name)}」${t('吗？该操作会删除项目下的接口、环境、链路和执行记录。')}`,
    okText: t('删除'),
    okType: 'danger',
    cancelText: t('取消'),
    async onOk() {
